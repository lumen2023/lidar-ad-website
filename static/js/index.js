...
function setupVideoSwitcher() {
    const cards = document.querySelectorAll('.case-video-card');
    if (cards.length === 0) return;

    cards.forEach(function(card) {
        const video = card.querySelector('.case-video');
        const title = card.querySelector('.case-video-title');
        const buttons = card.querySelectorAll('.case-choice');
        ...
        buttons.forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                video.play().catch(function() {
                    // Controls remain visible when autoplay is blocked.
                });
            });
        });
    });
